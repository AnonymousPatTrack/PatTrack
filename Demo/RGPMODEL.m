classdef RGPMODEL < handle
    ...
    methods
        function obj = RGPMODEL(X, Y, hyper)
        ...
        function pred = predict(obj, x, X, sizey)
            ...
            for i = 1:size(x,2)
  %              disp("aaa");
   %             disp(size(obj.kernel(X, x(:,i)),1));
    %            disp(size(obj.kernel(X, x(:,i)),2));
     %           disp(size(obj.alpha,1));
      %          disp(size(obj.alpha,2));
